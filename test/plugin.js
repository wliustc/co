    ...
    system = {
        systemTime: {
            load: function(callback) {
                var that = this;
                $.getJSON('//pub.' + doMain + '/clock/datetime?type=2&callback=?', function(data) {
                    that.timeDiff = new Date().getTime() - data.time;
        ...
        seckillService: function(options, componentOpt, callBack) {
            options = $.extend({
                ...
                prdsHtml: function(item, status, shopUrl, urlParam, actCode) {
                    var btnTemp = "";
                    if (item.goodCss == "no_prd" || status.isEnd) btnTemp = '<a href="javascript:void(0)" data-id="' + item.listId + '" class="over">宸茬鍏�</a>';
                    else if (status.share) btnTemp = '<a href="' + shopUrl + item.listId + '/' + actCode + urlParam + '" data-id="' + item.listId + '" class="share">鍒嗕韩绔嬭幏绉掓潃璧勬牸</a>';
                    else if (item.goodCss != "no_prd" && status.isActive) btnTemp = '<a href="' + shopUrl + item.listId + '/' + actCode + urlParam + '" data-id="' + item.listId + '" class="begin">绔嬪嵆绉掓潃</a>';
                    else btnTemp = '<a href="' + shopUrl + item.listId + '/' + actCode + urlParam + '"  data-id="' + item.listId + '"  class="nobegin">鍗冲皢寮€濮�</a>';
                    var tmp = '<li><a href="' + shopUrl + item.listId + '/' + actCode + urlParam + '"><img src="' + item.itemImgUrl.replace(/^http(s)?:/, '') + '"></a><p class="desc">' + item.itemName + '</p><p class="price"><span class="r_price">楼' + item.salPrice + '</span><span class="cg_price">绉掓潃浠凤細楼<label>' + item.seckillPrice + '</label></span></p><p class="count"><span>鏁伴噺: 200</span></p>' + btnTemp + '</li>';
                    return tmp
                },
            ...
            var components = {
                ...
                timeLine: {
                    ...
                    timeLineHtml: (componentOpt.timeLine && componentOpt.timeLine.timeLineHtml) || function(i, time, code) {
                        var tmp = '<li class="time ' + (i == 0 ? "focus" : "") + '" data-val="' + code + '">' + new Date(time.replace(/-/g, '/')).format('hh:nn') + '鍦�</li>';
                        return tmp
                    },
                ...
                countDown: {
                    ...
                    htmlTemp: '',
                    timeFormat: (componentOpt.countDown && componentOpt.countDown.timeFormat) || ['澶�', '鏃�', '鍒�', '绉�'],
                    distanceEndText: (componentOpt.countDown && componentOpt.countDown.distanceEndText) || '璺濈粨鏉�',
                    distanceStartText: (componentOpt.countDown && componentOpt.countDown.distanceStartText) || '璺濆紑濮�',
                    render: function(html) {
                        var oThis = this;
                        $(oThis.container).append(html)
                    },
                    ...
                    if (dates.nowTime < dates.startTime) {
                        ...
                        bindCountDown(countElm, dates.startTime, session, function() {
                            $('.nobegin').each(function() {
                                $(this).attr("class", "ms_btn begin").html('绔嬪嵆绉掓潃')
                            });
                            ...
                            bindCountDown(countElm, dates.endTime, session, function() {
                                renderCountDown('绉掓潃宸茬粨鏉�', session, actCode);
                                $('.wrapUl .ms_btn').attr("class", "ms_btn over").attr("href", "javascript:void(0)").html("宸茬鍏�")
                            })
                        })
            ...
            var render = function(session, prds, status, actCodeInfo) {
                ...
                for (var i = 0; i < prds.length; i++) {
                    ...
                    if (options.pageNum > 1) {
                        ...
                        if (i == prds.length - 1) {
                            prdHtml = prdHtml + options.prdsHtml(item, status, shopUrl, urlParam, actCode) + '</ul><div class="loaddiv" style="display: none"><span>鍔犺浇涓�...</span></div>'
                        } else {
                            prdHtml += options.prdsHtml(item, status, shopUrl, urlParam, actCode)
                        }
                    }
